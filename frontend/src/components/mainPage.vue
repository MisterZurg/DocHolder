        ...
        <md-table v-model="searched" md-sort="name" md-sort-order="asc" md-card md-fixed-header>
            <md-table-toolbar>
                ...
                <md-field md-clearable class="md-toolbar-section-end">
                    <md-input placeholder="Search company by name..." v-model="search"></md-input>
                </md-field>
                <md-button class="md-toolbar-section-end search-button"
                           @click="updateTable()"
                >Search
                </md-button>
            </md-table-toolbar>
    ...
    methods: {
        ...
        getCompanies() {
            ...
            this.$http(
                {
                    method: 'get',
                    url: 'http://localhost:8082/company/published/filter?limit=' + this.companiesPerPage + '&page=' + (this.pageNumber - 1) + '&searchName=' + this.search,
                    headers: {
        ...
        getPageColumn() {
            var query = this.$http(
                {
                    method: 'get',
                    url: 'http://localhost:8082/company/countPublished&name=' + this.search,
                    headers: {
                        "Content-type": "application/json; charset=UTF-8"
                    }
                })
